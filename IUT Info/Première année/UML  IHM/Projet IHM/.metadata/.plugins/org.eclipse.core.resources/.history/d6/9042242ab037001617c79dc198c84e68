...
public class EventsForm {
    ...
    void toNextPage(ActionEvent event) {
    	if (checkIntegration.isSelected() == true)
    		this.currentStudent.addEvent(new Evenement("Week End d'Intégration"));
    	if (checkPolytech.isSelected() == true)
    		this.currentStudent.addEvent(new Evenement("Soirée Polytech"));
    	if (checkYearEnd.isSelected() == true)
    		this.currentStudent.addEvent(new Evenement("Soirée de fin d'année"));
    	try {
